...
export function* callSaga(action) {
  try {
    const {opponentsIds, screen, type} = action.payload
    const session = yield call(QB.webrtc.call, {opponentsIds, type})
...
function* redirectToCallScreen(action) {
  const { screen } = action.payload.session
  Navigation.navigate('TeledateDial', {screen})
}

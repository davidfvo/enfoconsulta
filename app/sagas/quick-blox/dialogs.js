...
export function* getDialogs(action = {}) {
  try {
    ...
    const savedDialogs = yield select(({ QBdialogs }) => QBdialogs.dialogs)
...
export function* createDialog(action = {}) {
  try {
    ...
    const dialog = yield call(QB.chat.createDialog, request)
    dialog.color = getRandomColor()
...
export function* leaveDialog(action = {}) {
  const { dialogId } = action.payload
  try {
    const { dialogs, user } = yield select(({ QBauth, QBdialogs }) => ({
